...
if __name__ == "__main__":
    ...
    with plt.style.context( ('../../thesis_classic.mplstyle') ):
        ...
        for i in range( args.n + 1 ):
            ...
            plt.plot( [x/(len(lbRho)-1) for x in range(len(lbRho))], [x*12 for x in lbRho], color=colorList[i], label = r'$a = {:.2f}$'.format(aList[i]))
        ...
        if args.png == True:
            ...
            plt.savefig( 'rhor_vdWColumnHT_a_eos.png', format='png', bbox_inches = 'tight', dpi=600 )

        else:

            plt.savefig( 'rhor_vdWColumnHT_a_eos.pdf', format='pdf', bbox_inches = 'tight', dpi=600 )
